...
For example we are interested how `t` becomes `ծ` (we know `t` can become `տ`, `թ` or `ծ`). We now that it usually happens when `t` appears in a bigram `ts`, which should be converted to `ծ_`. 
...
Our experiments showed that too many neurons are involved in "decision making" even for the simplest cases, but it is possible to identify a subset of neurons that have more influence than the others. On the other hand, most neurons are involved in multiple decision making processes depending on the context. This is expected, because nothing in the loss functions we use in training neural nets forces the neurons to be independent and interpretable. Recently, there are [some attempts](https://arxiv.org/abs/1606.03657) to apply information-theoretic regularization terms in order to obtain more interpretability. It would be interesting to test those ideas in the context of transliteration.