...
When RNN size is only `128`, we notice that the best performance is achieved when dropout is `0.2`. Larger dropout values do not allow the network to learn enough. For RNN size `256` the optimal dropout value is somewhere between `0.2` and `0.4`. For RNN size 512 
